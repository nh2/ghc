...
currentCallStack = getCurrentCCS () >>= ccsToStrings
...
ccsToStrings ccs0 = go ccs0 []
  where
    go ccs acc
     ...
     | otherwise = do
        cc  <- ccsCC ccs
        lbl <- ccLabel cc >>= GHC.peekCString utf8
        mdl <- ccModule cc >>= GHC.peekCString utf8
        loc <- ccSrcSpan cc >>= GHC.peekCString utf8
...
errorWithStackTrace x = unsafeDupablePerformIO $ do
   stack <- getCurrentCCS x >>= ccsToStrings
   if null stack
