 ...
 * or the GHC RTS's per-thread `rts_interruptOSThreadEvent()`.
 *
...
WaitForObjectOrThreadInterrupt(HANDLE hHandle, DWORD dwMilliseconds)
{
    ...
    hWaits[0] = hHandle;
    hWaits[1] = rts_interruptOSThreadEvent();
...
fdReady(int fd, bool write, int64_t msecs, bool isSock)
{
    ...
    if (isSock) {
        ...
        // `rts_interruptOSThreadEvent()` event HANDLE for the
            ...
            case FILE_TYPE_CHAR:
                {
                    ...
                    // `rts_interruptOSThreadEvent()`, -1 is returned straight
                    // away.
